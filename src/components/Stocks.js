  ...
  const getDailyChartForStock = (symbol) => {
    return axiosInstance.get("", {
        params: {
            ...
            symbol,
            apikey: `${process.env.REACT_APP_STOCK_API_KEY_ALPHA}`
        }
    })
}
...
const handleSubmit = (data) => {
    console.log(input)
    console.log(data)
    let openCloseByDate = `https://api.polygon.io/v1/open-close/${input.input}/${input.date}?adjusted=true&apiKey=${process.env.REACT_APP_STOCK_API_KEY_POLYGON}`
    data.preventDefault();
